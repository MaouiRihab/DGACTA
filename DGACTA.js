...
function legend() {
    ...
    overlays.getLayers().getArray().slice().forEach(layer => {
        ...
        element.appendChild(head);
        var img = new Image();
        img.src = "http://localhost:8088/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=" + layer.get('title');
...
$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "http://localhost:8088/geoserver/wfs?request=getCapabilities",
        dataType: "xml",
...
$(function() {
    $("#layer").change(function() {
        ...
        $(document).ready(function() {
            $.ajax({
                type: "GET",
                url: "http://localhost:8088/geoserver/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" + value_layer,
                dataType: "xml",
                success: function(xml) {
...
$(function() {
    $("#attributes").change(function() {
        ...
        if (value_type == 'xsd:short' || value_type == 'xsd:int' || value_type == 'xsd:double' || value_type == 'xsd:long') {
            ...
            operator1.options[1] = new Option('Supérieur à', '>');
            operator1.options[2] = new Option('Inférieur à', '<');
            operator1.options[3] = new Option('Egal à', '=');
...
function query() {
    ...
    var url = "http://localhost:8088/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "%20" + value_operator + "%20" + value_txt + "&outputFormat=application/json"
    //console.log(url);
...
// highlight the WMS layers in the WMS layers table
function addRowHandlers1() {
    ...
    // Trouver l'index de la colonne "Nom de la ville"
    for (var i = 0; i < heads.length; i++) {
        if (heads[i].innerHTML === 'Nom de la ville') { // Changer 'Name' à 'Nom de la ville'
            col_no = i + 1; // Ajouter 1 pour obtenir un index basé sur 1
            break; // Sortir de la boucle une fois que nous avons trouvé la colonne
        }
    }

    // Vérification si col_no a été trouvé
    if (!col_no) {
        console.error('Colonne "Nom de la ville" non trouvée.'); // Message d'erreur
        return; // Sortir de la fonction si la colonne n'est pas trouvée
    }
    ...
    for (var i = 0; i < rows.length; i++) {
        rows[i].onclick = function() {
            return function() {
                ...
                var cell = this.cells[col_no - 1]; // Récupérer la cellule

                // Vérifier si la cellule existe
                if (cell) {
                    let layer_name = cell.innerHTML; // Récupérer le nom de la couche
                    ...
                    // Ajouter la couche sélectionnée à la carte
                    add_layer(layer_name); 
                } else {
                    console.error('La cellule n\'existe pas pour la ligne cliquée.');
                }
            };
        }(rows[i]);
    }
...
// list WMS layers in window on click of button
function wms_layers() {
    $(function() {
        $("#wms_layers_window").modal({ backdrop: false }).draggable().modal('show');
    });

    $(document).ready(function() {
        $.ajax({
            type: "GET",
            url: "http://localhost:8088/geoserver/wms?request=getCapabilities",
            dataType: "xml",
            success: function(xml) {
                $('#tableBodyWMS').empty(); // Vider le corps du tableau avant de le remplir
                $(xml).find('Layer').find('Layer').each(function() {
                    var name = $(this).children('Name').text();
                    var title = $(this).children('Title').text();
                    var abst = $(this).children('Abstract').text();
                    var city = getCityForLayer(name); // Associer une ville

                    // Ajouter une ligne pour chaque couche
                    $('<tr></tr>').html(
                        '<td>' + city + '</td>' +
                        '<td>' + name + '</td>' +
                        '<td>' + title + '</td>' +
                        '<td>' + abst + '</td>'
                    ).appendTo('#tableBodyWMS');
                });

                addRowHandlers1(); // Ajouter les handlers après la création des lignes
            }
        });
    });
}
...
// Assign a city to the WMS layer based on its name
function getCityForLayer(layerName) {
    // Vérification des mots-clés dans le nom des couches pour déterminer la ville
    if (layerName.toLowerCase().includes('tunis')) {
        return 'Tunis';
    } else if (layerName.toLowerCase().includes('bizerte')) {
        return 'Bizerte';
    } else if (layerName.toLowerCase().includes('sousse')) {
        return 'Sousse';
    } else if (layerName.toLowerCase().includes('monastir')) {
        return 'Monastir';
    } else if (layerName.toLowerCase().includes('gabes')) {
        return 'Gabes';
    } else if (layerName.toLowerCase().includes('ariana')) {
        return 'Ariana';
    } else if (layerName.toLowerCase().includes('siliana')) {
        return 'Siliana';
    } else if (layerName.toLowerCase().includes('sfax')) {
        return 'Sfax';
    } else if (layerName.toLowerCase().includes('beja')) {
        return 'Beja';
    } else if (layerName.toLowerCase().includes('gafsa')) {
        return 'Gafsa';
    } else if (layerName.toLowerCase().includes('medenine')) {
        return 'Medenine';
    } else if (layerName.toLowerCase().includes('tataouine')) {
        return 'Tataouine';
    } else if (layerName.toLowerCase().includes('kairouan')) {
        return 'Kairouan';
    } else if (layerName.toLowerCase().includes('nabeul')) {
        return 'Nabeul';
    } else if (layerName.toLowerCase().includes('kef')) {
        return 'Le Kef';
    } else if (layerName.toLowerCase().includes('zaghouan')) {
        return 'Zaghouan';
    } else if (layerName.toLowerCase().includes('kasserine')) {
        return 'Kasserine';
    } else if (layerName.toLowerCase().includes('tozeur')) {
        return 'Tozeur';
    } else if (layerName.toLowerCase().includes('manouba')) {
        return 'Manouba'; 
    } else if (layerName.toLowerCase().includes('ben_arous')) {
        return 'Ben_Arous'; 
    } else if (layerName.toLowerCase().includes('gafsa')) {
        return 'Gafsa'; 
    } else if (layerName.toLowerCase().includes('jendouba')) {
        return 'Jendouba';
    } else if (layerName.toLowerCase().includes('mahdia')) {
        return 'Mahdia'; 
    } else if (layerName.toLowerCase().includes('sidi_bouzid')) {
        return 'Sidi_Bouzid';
    } else if (layerName.toLowerCase().includes('carte_globale')) {
        return 'Carte_globale'; 
    } else if (layerName.toLowerCase().includes('kebili')) {
        return 'Kebili'; 
    } else {
        return 'Autre Ville';
    }
...
// Filter the WMS layers by city
function filterWMSLayersByCity() {
    const searchInput = document.getElementById('searchWMSInput').value.toLowerCase();
    const tableRows = document.getElementById('tableBodyWMS').getElementsByTagName('tr');
    for (let i = 0; i < tableRows.length; i++) {
        let cityCell = tableRows[i].getElementsByTagName('td')[0];
        let cityText = cityCell.textContent.toLowerCase();
        if (cityText.includes(searchInput)) {
...
function add_layer(layer_name) {
    console.log("Nom de la couche passé à add_layer :", layer_name); // Devrait afficher le nom de la couche

    if (!layer_name) {
        console.error("layer_name est indéfini !");
        return; // Arrête l’exécution si le nom est manquant
    }

    var name = layer_name.split(":");
    console.log("Nom de la couche après split :", name);
    ...
    console.log("Couche ajoutée avec succès :", layer_name);
}


// Close the WMS layers window
...
function clear_all() {
    ...
    map.getView().fit([10.0, 36.7, 10.3, 36.9], {
        duration: 1590,
        size: map.getSize()
    });
    ...
    if (value !== 'None') {
		...
		draw1 = new ol.interaction.Draw({
            ...
            draw1.on('drawend', function(evt) {
                ...
                var url = "http://localhost:8088/geoserver/wfs?request=GetFeature&version=1.0.0&typeName=" + value_layer + "&outputFormat=json&cql_filter=INTERSECTS(the_geom," + wkt + ")";
                //alert(url);
...
function addInteraction() {
    ...
    if (measuretype.value == 'select' || measuretype.value == 'clear') {
        ...
        var pointerMoveHandler = function(evt) {
            ...
            /** @type {string} */
            var helpMsg = 'Cliquez pour commencer à dessiner';
...
var filter
function filterCityLayers() {
    const searchInput = document.getElementById('searchWMSInput').value.toLowerCase();
    const tableBody = document.getElementById('tableBodyWMS');
    
    // Effacer le contenu précédent du tableau
    tableBody.innerHTML = '';

    // Filtrer les couches en fonction de la ville
    const filteredLayers = wmsLayers.filter(layer => layer.city.toLowerCase().includes(searchInput));
    console.log(filteredLayers)

    // Parcourir les couches filtrées et les ajouter dans le tableau
    filteredLayers.forEach(layer => {
        const row = document.createElement('tr');
        
        // Créer les cellules pour la ville et la couche
        const cityCell = document.createElement('td');
        cityCell.textContent = layer.city;

        const layerCell = document.createElement('td');
        layerCell.textContent = layer.layer;
...
// Appel initial pour afficher toutes les couches
filterCityLayers();
